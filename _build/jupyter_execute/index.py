...
try:
    ...
    strain_H1, time_H1, chan_dict_H1 = ligo.loaddata(fn_H1, 'H1')
    strain_L1, time_L1, chan_dict_L1 = ligo.loaddata(fn_L1, 'L1')
except:
...
if whiten_data:
    ...
    strain_H1_whiten = ligo.whiten(strain_H1,psd_H1,dt)
    strain_L1_whiten = ligo.whiten(strain_L1,psd_L1,dt)
...
template_p_whiten = ligo.whiten(template_p,psd_H1,dt)
template_c_whiten = ligo.whiten(template_c,psd_H1,dt)
...
for det in dets:
    ...
    template_whitened = ligo.whiten(template_rolled,interp1d(freqs, data_psd),dt)  # whiten the template
...
#plot variables around event 

saveplots = 1
plot = ligo.plot_dets(make_plots, saveplots, eventname, 'png')
...
# write the files:
ligo.write_wavfile('audio/'+eventname+"_H1_whitenbp.wav",int(fs), strain_H1_whitenbp[indxd])
ligo.write_wavfile('audio/'+eventname+"_L1_whitenbp.wav",int(fs), strain_L1_whitenbp[indxd])

# re-whiten the template using the smoothed PSD; it sounds better!
template_p_smooth =ligo.whiten(template_p,psd_smooth,dt)
...
ligo.write_wavfile('audio/'+eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
strain_H1_shifted = ligo.reqshift(strain_H1_whitenbp,fshift=fshift,sample_rate=fs)
strain_L1_shifted = ligo.reqshift(strain_L1_whitenbp,fshift=fshift,sample_rate=fs)

# write the files:
ligo.write_wavfile('audio/'+eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
ligo.write_wavfile('audio/'+eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = ligo.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
ligo.write_wavfile('audio/'+eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
...
if data_segments:
    ...
    strain, time, chan_dict = ligo.loaddata(fn_L1, 'H1')
    ...
    segment_list = ligo.dq_channel_to_seglist(chan_dict[DQflag])
    ...
    segment_list = ligo.dq_channel_to_seglist(chan_dict['NO_CBC_HW_INJ'])
    print('Number of segments with DQflag',DQflag,' = ',len(segment_list))
    iseg = 0
    for segment in segment_list:
        time_seg = time[segment]
